...
gulp.task("copyToDemo", function() {
  return gulp.src(["app/**/*", "!app/sass/**/*"]).pipe(gulp.dest("docs")); // Create docs folder for gitHub page to show demo
});

gulp.task("cleanDemoDocsFolder", function() {
  return gulp.src("docs/", { read: false, allowEmpty: true }).pipe(clean());
});
...
if (gulpVersion == 3) {
  gulp.task(
    "watch",
    ["styles", "stylesMin", "scripts", "browser-sync"],
    function() {
      gulp.watch("app/sass/**/*.+(scss|sass)", ["styles", "stylesMin"]);
      gulp.watch(["libs/**/*.js", "app/js/common.js"], ["scripts"]);
      gulp.watch(["app/*.html", "app/js/*.js"], ["code"]);
    }
  );
...
if (gulpVersion == 4) {
  gulp.task("watch", function() {
    gulp.watch(
      "app/sass/**/*.+(scss|sass)",
      gulp.parallel(["styles", "stylesMin"])
    );
